...
function LibraryOverviewView({ onOpenGroup, onAction }: { onOpenGroup: (groupId: LibraryGroupId) => void; onAction: (message: string) => void }) {
  const groupMetrics: Record<LibraryGroupId, { sources: number; health: string }> = {
    projects: { sources: 16, health: "82%" },
    organizations: { sources: 26, health: "88%" },
    decisions: { sources: 7, health: "91%" },
    timeline: { sources: 14, health: "79%" },
    methods: { sources: 10, health: "86%" },
    pending: { sources: 12, health: "62%" },
  };
    ...
    <section className="library-overview-view">
      <header className="library-summary-card">
        <div>
          ...
          <h1>我的20年工作档案</h1>
          <p>持续整理项目、客户、决策与方法，把零散资料重构成可追溯、可复用的长期知识。</p>
        </div>
        ...
        <div className="library-directory-list">
          {libraryGroups.map((group) => (
            <button key={group.id} onClick={() => onOpenGroup(group.id)}>
              ...
              <em>{groupMetrics[group.id].health}</em>
...
function ImportView({ onAction }: { onAction: (message: string) => void }) {
  ...
  const previewRows = [
    ["2019_A公司项目复盘.docx", "Word", "已解析", "已合并", "A 公司数字化项目", "8 段"],
    ["2020_B公司会议纪要.pdf", "PDF", "已解析", "重构中", "B 公司流程优化", "3 段"],
    ["数字化咨询方案.pptx", "PPT", "已解析", "已合并", "A 公司数字化项目", "6 段"],
    ["合同扫描件_补充条款.png", "图片", "OCR 待确认", "待证据", "合同与报价证据链", "1 段"],
  ];
    ...
    <section className="import-view">
      <div className="import-main">
        <section className="import-hero">
          ...
          <section className="import-simple-card" aria-label="导入资料">
            ...
            <section className="import-quick-settings">
              ...
              <div className="import-quick-settings-grid">
                <label>
                  <span>导入到</span>
                  <select
                    value={targetMode === "new" ? "__new__" : existingLibrary}
                    onChange={(event) => {
                      if (event.target.value === "__new__") setTargetMode("new");
                      else { setTargetMode("existing"); setExistingLibrary(event.target.value as keyof typeof libraries); }
                    }}
                  >
                    {Object.entries(libraries).map(([value, label]) => <option value={value} key={value}>{label}</option>)}
        ...
        <section className="import-middle-grid">
          <article className="import-process-card">
            <h2>导入后会发生什么</h2>
            <div>
              <span><strong>1. Source 入库</strong><small>记录文件、URL、文本或 repo 的元数据和原始位置。</small></span>
              <span><strong>2. 解析与清洗</strong><small>转换为规范文本，并保留页码、段落和块锚点。</small></span>
              <span><strong>3. 知识重构</strong><small>把多份来源中描述同一主题的内容合并为知识对象。</small></span>
              <span><strong>4. 引用与 MCP</strong><small>生成带引用的知识页，并同步到搜索、图谱和只读 MCP。</small></span>
            </div>
          </article>

          <article className="import-batch-card">
            <h2>当前上传批次</h2>
            <div className="import-progress-head"><span>正在上传</span><strong>72%</strong></div>
            <div className="import-progress"><i /></div>
            <p>合同扫描件_补充条款.png</p>
            <div className="import-queue"><strong>处理队列</strong><span><Clock size={14} />客户会议纪要.pdf</span><span><Clock size={14} />产品方案.pptx</span><span><Clock size={14} />合同扫描件.png</span></div>
          </article>
        </section>
        ...
        <section className="import-preview-card">
          ...
          <div className="import-preview-table" role="table" aria-label="导入预检结果">
            <div className="import-preview-row head" role="row"><span>Source</span><span>类型</span><span>解析</span><span>重构</span><span>关联知识页</span><span>引用使用</span></div>
            {previewRows.map((row) => <div className="import-preview-row" role="row" key={row[0]}>{row.map((cell, index) => <span key={`${cell}-${index}`}>{index === 0 && <FileText size={14} />}{cell}</span>)}</div>)}
          </div>
        </section>
      </div>
  ...
  return (
    <section className="source-files-view">
      <header className="source-files-heading">
        <div><p>我的20年工作档案</p><h1>原始文件</h1><span>保留未经重构的文件、网页与资源包，作为知识页的可追溯证据来源。</span></div>
        <button onClick={onImport}><UploadSimple size={16} weight="regular" />导入新资料</button>
      </header>
      <div className="source-file-stats">
        <span><small>全部文件</small><strong>1,284</strong></span>
        <span><small>已解析</small><strong>1,261</strong></span>
        <span><small>处理中</small><strong>3</strong></span>
        <span><small>待确认</small><strong>20</strong></span>
      </div>
      <div className="source-file-table">
        ...
        {files.map((file) => (
          <button className="source-file-row" key={file.name}>
            ...
            <span>{file.type}</span><span>{file.size}</span><span><em className={file.status === "待确认" ? "review" : ""}>{file.status}</em></span><span>{file.target}</span><span>{file.time}</span>
          </button>
        ))}
      </div>
    </section>
  ...
  return (
    <section className="knowledge-graph-view">
      <header className="knowledge-graph-heading">
        <div><p>我的个人知识库 / 工作台</p><h1>知识图谱</h1></div>
        <span><ShieldCheck size={16} weight="regular" />私有</span>
      </header>

      <div className="knowledge-graph-content">
        <div className="knowledge-graph-filters" role="group" aria-label="筛选知识图谱">
          {filters.map((item) => (
            <button
              type="button"
              key={item.id}
          ...
          <div className="profile-center-grid profile-settings-grid">
            ...
            <section className="profile-card profile-appearance">
              ...
              <div className="profile-theme-options" role="group" aria-label="界面主题">
                <button type="button" className={theme === "light" ? "active" : ""} onClick={() => onThemeChange("light")}><span>浅色</span><small>明亮、清晰</small></button>
                <button type="button" className={theme === "dark" ? "active" : ""} onClick={() => onThemeChange("dark")}><span>深色</span><small>低光环境</small></button>
              </div>
              <small>主题偏好会保留在当前设备。</small>
            </section>
          </div>
        </div>
      </section>
...
function WorkspaceDashboard({ onOpenLibrary, onNewLibrary, onOpenImport, onAction }: { onOpenLibrary: () => void; onNewLibrary: () => void; onOpenImport: () => void; onAction: (message: string) => void }) {
  ...
  const stats = [
    { label: "知识文件", value: "1,284", icon: FolderOpen },
    { label: "知识页", value: "96", icon: FileText },
    { label: "引用覆盖", value: "81%", icon: LinkSimple },
    { label: "最近更新", value: "10:42", icon: Clock },
  ];
  const knowledgeBases = [
    { title: "我的20年工作档案", description: "工作、研究与项目资料", meta: "1,284 个文件 · 96 篇知识页 · 今天 10:42", state: "重构中", tone: "building", current: true },
    { title: "产品研究资料库", description: "竞品、行业报告与访谈", meta: "248 个文件 · 31 篇知识页 · 昨天 18:20", state: "可用", tone: "ready" },
    { title: "客户项目库", description: "项目方案、会议与交付资料", meta: "86 个文件 · 14 篇知识页 · 7 月 30 日", state: "需确认", tone: "review" },
    { title: "课程与论文资料库", description: "课程资料、论文与研究笔记", meta: "174 个文件 · 28 篇知识页 · 7 月 28 日", state: "可用", tone: "ready" },
    { title: "个人内容素材库", description: "选题、案例与长期观点", meta: "132 个文件 · 22 篇知识页 · 7 月 26 日", state: "可用", tone: "ready" },
  ];
  const visibleLibraries = knowledgeBases.filter((library) => `${library.title}${library.description}`.toLowerCase().includes(librarySearch.trim().toLowerCase()));
      ...
      <section className="workspace-progress-card">
        <header className="workspace-hub-heading">
          ...
          <div className="workspace-hub-actions">
            ...
            <button className="workspace-view-button" onClick={onOpenLibrary}>查看全部知识页 <ArrowRight size={17} weight="bold" /></button>
          </div>
        </header>
        ...
        <div className="workspace-rebuild-progress">
          <div><span><strong>当前知识库正在重构</strong><small>18 份资料正在合并为 3 篇知识页</small></span><strong>72%</strong></div>
          <span className="workspace-progress-track"><i /></span>
        </div>
      </section>

      <section className="workspace-libraries-card">
        <header className="workspace-libraries-heading">
          <div><span>MY KNOWLEDGE BASES</span><h2>我的知识库</h2></div>
          <button onClick={onNewLibrary}><Plus size={18} weight="regular" />新建知识库</button>
        </header>

        <div className="workspace-library-toolbar">
          <span>共 12 个知识库</span>
          <label><MagnifyingGlass size={17} weight="regular" /><input value={librarySearch} onChange={(event) => setLibrarySearch(event.target.value)} placeholder="搜索知识库" aria-label="搜索知识库" /></label>
        </div>

        <div className="workspace-library-list" aria-label="我的知识库列表">
          {visibleLibraries.map((library) => (
            <button className={library.current ? "current" : ""} key={library.title} onClick={() => library.current ? onOpenLibrary() : onAction(`已选择${library.title}`)}>
              <span className="workspace-library-icon"><Books size={21} weight="regular" /></span>
              <span className="workspace-library-copy"><strong>{library.title}</strong><small>{library.description}</small><em>{library.meta}</em></span>
              <span className={`workspace-library-state ${library.tone}`}>{library.state}</span>
              <CaretRight size={18} weight="bold" />
            </button>
          ))}
          {visibleLibraries.length === 0 && <div className="workspace-library-empty">没有找到匹配的知识库</div>}
        </div>
      </section>
    </div>
  );
  ...
  return (
    <div className="kf-home">
      <header className="kf-home-header">
        <a href="#home" className="kf-home-brand" aria-label="KnowForge 首页">
          <BrandMark />
          <span><strong>KnowForge</strong><small>AI 知识重构工作台</small></span>
        </a>
      ...
      <footer className="kf-home-footer"><span>KnowForge</span><small>把原始资料编译为可持续使用的可信知识。</small></footer>
    </div>
  );
}
...
function KnowForgeLanding({ onEnter }: { onEnter: () => void }) {
  ...
  return (
    <div className="kf-home">
      ...
      <footer className="kf-home-footer"><div><BrandMark /><span><strong>KnowForge</strong><small>AI 知识重构工作台</small></span></div><nav><a href="#workflow">工作方式</a><a href="#scenarios">使用场景</a><a href="#capabilities">产品能力</a></nav><small>把原始资料编译为可持续使用的可信知识。</small></footer>
    </div>
  );
}
...
export function KnowForgeDemo() {
  ...
  return (
    <div className={`site ${currentView === "dashboard" ? "dashboard-active" : ""}`} data-theme={theme}>
      <header className="topbar">
        <a className="brand" href="#overview" aria-label="KnowForge 首页" onClick={(event) => { event.preventDefault(); openDocs(); }}>
          <BrandMark />
          <span>knowforge</span>
        </a>
      ...
      <aside className={`left-sidebar ${mobileNavOpen ? "open" : ""}`}>
        <div className="sidebar-branding">
          <BrandMark />
          <div><strong>KnowForge</strong><span>AI 知识重构工作台</span></div>
        </div>
